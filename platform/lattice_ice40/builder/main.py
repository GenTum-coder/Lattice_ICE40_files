...
elif upload_protocol == "olimex":

    env.Replace(
        UPLOADER = "iceprogduino",
        UPLOADERFLAGS = ["-I", "$UPLOAD_PORT"],
    )
...
vcd = Builder(
    ...
    action='toolbox run vvp {0} $SOURCE'.format(
        VVP_PATH),
    suffix='.vcd',
    src_suffix='.out')
...
# target_sim = env.Alias('sim', vcd_file, 'toolbox run gtkwave {0} {1}.gtkw'.format(
target_sim = env.Alias('sim', vcd_file, 'flatpak run io.github.gtkwave.GTKWave {0} {1}.gtkw'.format(
